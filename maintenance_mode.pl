...
# Globals
#====================================================
my $STR_Prgname   = 'maintenance_mode';
my $STR_Version   = '1.00';
my $STR_Edition   = '15/07/2019';
my $MNT_QUEUE     = Thread::Queue->new();
my $STR_NBThreads = 3;
my $CALLBACK_ADDR = "/AXA-IM_PROD/FR1AS1IT1-0047/FR1AS1IT1-0047/maintenance_mode";
...
sub managed {
    ...
    nimLog(3, "(managed) Handle server: $serverName");
    $MNT_QUEUE->enqueue({
...
sub unmanaged {
    my ($hMsg, $serverName) = @_;
    nimLog(3, "(unmanaged) Handle server: $serverName");
    $MNT_QUEUE->enqueue({
